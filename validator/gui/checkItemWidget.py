...
#
		...
		self.fixButton.setIcon(QIcon(os.path.join(dir, CHECK_ICON)))
		...
		self.arrow_down = QPixmap(os.path.join(dir, ARROWDOWN_ICON))
		self.arrow_right = QPixmap(os.path.join(dir, ARROWRIGHT_ICON))
...
class DataWidget(QWidget):
	def __init__(self, parent = None):
		...
		self.scrollArea.setStyleSheet("background-color: rgb(" + str(SCROLLAREA_COLOR[0]) + "," + str(SCROLLAREA_COLOR[1]) + "," + str(SCROLLAREA_COLOR[2]) + ");border: 0px solid")
...
class HelpButton(QWidget):
	def __init__(self, action = None, *args):
		...
		self.help_button.setIcon(QIcon(os.path.join(dir, QUESTION_ICON)))
...
class Browser(QWidget):
	def __init__(self, parent = mainWindowPointer(), url = None, title = None):
