...
class PresetButton(QPushButton):
	def __init__(self, scrollArrea, parent=None):
	...
	def _create_action(self):
		...
		for preset in PRESETS:
			...
			preset_menu.triggered.connect(lambda item = preset_menu.text(): self.setPreset(item))
			self.menu.addAction(preset_menu)
			self.group.addAction(preset_menu)

		return self.menu, self.group


	def setPreset(self, preset):
		self.setText('Preset: ' + preset)
		cmds.optionVar(sv = (PRESETOPTION, preset))
