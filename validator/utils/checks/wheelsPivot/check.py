...
def getCentroidOfFaces(faces):
    ...
    for idx, i in enumerate(finalPoints):
        if idx != endIdx:
            Cx += (finalPoints[idx][0] + finalPoints[idx + 1][0]) * (
                    finalPoints[idx][0] * finalPoints[idx + 1][1] - finalPoints[idx + 1][0] * finalPoints[idx][1])
        elif idx == endIdx:
            Cx += (finalPoints[idx][0] + finalPoints[0][0]) * (
                    finalPoints[idx][0] * finalPoints[0][1] - finalPoints[0][0] * finalPoints[idx][1])
    Cx = Cx / (6 * Area)

    for idx, i in enumerate(finalPoints):
        if idx != endIdx:
            Cy += (finalPoints[idx][1] + finalPoints[idx + 1][1]) * (
                    finalPoints[idx][0] * finalPoints[idx + 1][1] - finalPoints[idx + 1][0] * finalPoints[idx][1])
        elif idx == endIdx:
            Cy += (finalPoints[idx][1] + finalPoints[0][1]) * (
                    finalPoints[idx][0] * finalPoints[0][1] - finalPoints[0][0] * finalPoints[idx][1])
    Cy = Cy / (6 * Area)
...
def bboxWheels(wheel):
    ...
    hight = ("%.4f" % float(maximum[0][1] - minimum[0][1]))
    width = ("%.4f" % float(maximum[0][2] - minimum[0][2]))

    if (hight) == (width):
        h_center = (maximum[0][1] + minimum[0][1]) / 2
        w_center = (maximum[0][2] + minimum[0][2]) / 2

        if "%.4f" % h_center == "%.4f" % w_coord[1] and "%.4f" % w_center == "%.4f" % w_coord[2]:
            return True
        else:
            return False
    else:
        ...
        for w in listWheels:
            if w.find("lod0") != -1:
                if bboxWheels(w) == True:
                    ...
                    if ("%.4f" % w_coord[0]) != ("%.4f" % centroidCoords[0]) or ("%.4f" % w_coord[1]) != (
                            "%.4f" % centroidCoords[1]) or ("%.4f" % w_coord[2]) != ("%.4f" % centroidCoords[2]):
                        tmp = []
                        tmp.append(w + " - the pivot is not in center of wheel")
                        tmp.append(w)
                ...
                if lod0Wheels:
                    for i in lod0Wheels:
                        ...
                        if iShort == shortName:
                            ...
                            w_pivot = cmds.xform(w, q=1, ws=1, rp=1)
                            if round(obj_pivot[0], 4) != round(w_pivot[0], 4) or round(obj_pivot[1], 4) != round(
                                    w_pivot[1], 4) or round(obj_pivot[2], 4) != round(w_pivot[2], 4):
                                tmp = []
                                tmp.append(w + " - the pivot is not the same in LOD0")
                                tmp.append(w)
                                returnList.append(tmp)
