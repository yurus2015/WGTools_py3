    ...
    def checkNextLod(self, lodName): #get the polycount of the combination in the next lod
        ...
        for rel in relatives: #for each relative in lod0
            ...
            if self.turret:# if turret in lod0   (no, turret is None in lod0)
                if rel.find(turretSearchName) != -1 and rel.find("bsp") == -1: #no turret in all lods - lastFoundedTurret = None
        ...
        if lodName.find("lod1") != -1:
            ...
            if polycountResult > firstLod_policount:
                ...
                message = message + "Out of range with " +  str(polycountResult) + " tris > max polycount: " + str(firstLod_policount) + "\n"
            ...
            if polycountResult > secondLod_policount:
                ...
                # message = self.lastFoundedHull + " , " +  self.lastFoundedChassis + " , " +  self.lastFoundedTurret + " , " + self.lastFoundedGun + "\n    -"
                message = message + "Out of range with " +  str(polycountResult) + " tris > max polycount: " + str(secondLod_policount) + "\n"
            ...
            if polycountResult > thirdLod_policount:
                ...
                # message = self.lastFoundedHull + " , " +  self.lastFoundedChassis + " , " +  self.lastFoundedTurret + " , " + self.lastFoundedGun + "\n    -"
                message = message + "Out of range with " +  str(polycountResult) + " tris > max polycount: " + str(thirdLod_policount) + "\n"
                #self.lastPolycount = polycountResult
                return message
            else:
                #self.lastPolycount = polycountResult
                ...
                for comb in combinations_lod0:
                    ...
                    for index, lodet in enumerate(lodList):  #for each lod: lod1-lod3
                        if(index != 0 and index !=4):
                            result = foundedCombination.checkNextLod(lodet) #if checkNextLod returned false (in case current lod's polycount doesn't match the condition)
                            if result:
                                tmp = []
                                tmp.append(result)
                                tmp.append(result)
