    ...
    if len(lod0Tracks) > 1:
        ...
        def getLowestVertices(track):
            ...
            while not iterVertexes.isDone():
                ...
                if round(position.y,3)>= round(ground.y,3)-0.7 and round(position.y,3) <= round(ground.y,3)+0.7:
...
def messageBox(messageString):
    ...
    cmds.confirmDialog( title='Warning', message=messageString , button=['Ok'], defaultButton='Ok', cancelButton='No', dismissString='No' )
...
def main(*args):
    ...
    if abs(round(centerTank[0], 3)/100) > 0.001 or abs(round(centerTank[2], 3)/100) > 0.001:
        ...
        messageBox("Centered successfully X: " + str(round(centerTank[0]/100, 3)) + "; Z: " + str(round(centerTank[2]/100, 3))  )
    else:
