            ...
            if turretList:
                for t in turretList:
                    ...
                    if cmds.listRelatives(t, type="mesh", f=1):
                        ...
                        xDist = boundingBox[3] - boundingBox[0];
                        XPercentage = (xDist / 100) * pivotPositionThreshold;
                        zDist = boundingBox[5] - boundingBox[2];
                        ZPercentage = (zDist / 100) * pivotPositionThreshold;
                        if turretPivot[0] < (boundingBox[0] + XPercentage) or turretPivot[0] > (
                                boundingBox[3] - XPercentage):
                            tmp = []
                            tmp.append(t)
                            tmp.append(t)
                            returnList.append(tmp)
                        elif turretPivot[2] < (boundingBox[2] + ZPercentage) or turretPivot[2] > (
                                boundingBox[5] - ZPercentage):
                            tmp = []
                            tmp.append(t)
                            tmp.append(t)
                            returnList.append(tmp)
                        ...
                        if grpRelatives:
                            ...
                            if origPivot and havokPivot:
                                if not origPivot[0] == havokPivot[0] or not origPivot[1] == havokPivot[1] or not \
                                        origPivot[2] == havokPivot[2]:
                                    tmp = []
