...
def wrap_image(meshes, size = 1024):
	...
	cmds.polyWarpImage(xResolution = size, yResolution = size, inputName = WHITEFILE, outputName = snapshot, inputUvSetName = 'map1', outputUvSetName = 'map1', fileFormat = 'jpg', background = [0,0,0], overwrite = 1, noAlpha = 1)
...
def bounding_box_intersection(box_a, box_b):
	...
	if (abs(box_a[0] - box_b[0]) * 2 < (box_a[2] + box_b[2])) and (abs(box_a[1] - box_b[1]) * 2 < (box_a[3] + box_b[3])):
		return True
	else:
		return False
	# pass
