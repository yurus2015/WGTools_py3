...
class ToolOptions(QWidget):
    ...
    def createUI(self):
        ...
        self.label = QLabel(
            "<b>Description:</b><p>Adds rotation animation to all tracks found in the<br> current scene</p>")
    ...
    def main(self, update=0, channel=None):
        ...
        if not track_list_shape:  # if still no wheels
            cmds.inViewMessage(amg='<hl>Tracks were not found. Please select tracks manually.</hl>', pos='midCenter',
                               fade=True, fot=1000)
            return
        ...
        if cmds.play(q=1, state=1) == 0:
            ...
            if channel == "color":
                ...
                if not place2dTexture:
                    cmds.inViewMessage(amg='<hl>Assign color texture for moving control.</hl>', pos='midCenter',
                                       fade=True, fot=1000)
                    # continue
                    return
            elif channel == "normal":
                ...
                if not place2dTexture:
                    cmds.inViewMessage(amg='<hl>Assign normal texture for moving control.</hl>', pos='midCenter',
                                       fade=True, fot=1000)
                    # continue
                    return
            if place2dTexture:
