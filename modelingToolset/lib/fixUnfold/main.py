...
class ToolOptions(QWidget):
	...
	def main(self):
		...
		if not selection:
			cmds.inViewMessage(amg= '<hl>Please select one uv or an entire UV Shell</hl>' , pos = 'midCenter', fade = True, fot = 1000)
			return
		...
		if ".map[" not in selection[0]:
			cmds.inViewMessage(amg= '<hl>Please select one or more uvs to fix UV Shell</hl>' , pos = 'midCenter', fade = True, fot = 1000)
			return
