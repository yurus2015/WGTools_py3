...
def wrap_image(meshes, size = 1024):
	...
	cmds.polyWarpImage(xResolution = size*2, yResolution = size*2, inputName = WHITEFILE, outputName = snapshot, inputUvSetName = 'map1', outputUvSetName = 'map1', fileFormat = 'jpg', background = [0,0,0], overwrite = 1, noAlpha = 1)
...
class OpenCV(object):
	...
	def split_contours(self, contours, hierarchy_array):
		...
		for i in range(len(contours)):
			if hierarchy_array[i][3] > -1: #this contour is internal(has parent) (https://docs.opencv.org/3.4/d9/d8b/tutorial_py_contours_hierarchy.html)
