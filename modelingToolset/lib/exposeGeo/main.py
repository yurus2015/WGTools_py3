...
class ToolOptions(QWidget):
    ...
    def createUI(self):
        ...
        '''description'''
        self.label = QLabel(
            "<b>Description:</b><p>Moves geometry by assets and lods to make them visible apart from other assets/lods</p><br>")
            ...
            for lod in self.lodList:
                if len(lod) > 1:
                    for indx in range(1, len(lod)):
                        ...
                        asset_position = float(self.lods_slider.value()) * OFFSET_COEFF * (
                                    bbx[width] + bbx_[width]) * coeff + pos
    ...
    def assets_slider_changed(self):
        if len(self.assetList) > 1:
            ...
            for indx in range(1, len(self.assetList)):
                ...
                asset_position = float(self.assets_slider.value()) * OFFSET_COEFF * (
                            bbx[width] + bbx_[width]) * coeff + pos
            ...
            for y in range(num):
                for x in range(num):
                    if i < len(sort_list):
                        ...
                        cmds.setAttr(sort_list[i][0] + axis,
                                     y * self.havok_slider.value() * OFFSET_COEFF * coeff + offcet_position_x)
                        cmds.setAttr(sort_list[i][0] + anti_axis,
                                     x * self.havok_slider.value() * OFFSET_COEFF * coeff + offcet_position_z)
                        i = i + 1
        else:
            return
    ...
    def main(self):
        ...
        cmds.inViewMessage(amg='<hl>Please open Options to apply Expose Geometry</hl>', pos='midCenter', fade=True,
                           fot=1000)
