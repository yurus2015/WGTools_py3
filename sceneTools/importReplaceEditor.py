...
class Utils(object):
	...
	def referenceLoadUnload(cls, file):
		...
		if file_short in file:
			cmds.confirmDialog( title='Warning', message= 'Current file opened', button=['   OK   '], defaultButton='   OK   ')
		else:
			cmds.file(file, reference=True, namespace='r')
			filters = cmds.itemFilter(bn=('*:*'), neg=1)
			cmds.outlinerEditor('outlinerPanel1', e=1, showReferenceNodes=False, showReferenceMembers=False, filter = filters)
	...
	def importAction(cls):
		...
		if sel:
			...
			cmds.confirmDialog( title='Warning', message= 'Select object(s) from left panel', button=['   OK   '], defaultButton='   OK   ')
	...
	def replaceAction(cls):
		...
		if len(pair) == 2:
			if 'r:' in pair[0] and 'r:' not in pair[1]:
				cmds.connectAttr(pair[0] + '.outMesh', pair[1] + '.inMesh', f=1) # -f r:gun_02_Shape.outMesh gun_02_Shape.inMesh;
		else:
			cmds.confirmDialog( title='Warning', message= 'Select one object from left panel and one from right panel', button=['   OK   '], defaultButton='   OK   ')
	...
	def source_folder(cls, two_up_folder, folder_name):
		...
		if srs_folder:
			...
			cmds.confirmDialog( title='Warning', message= 'Directory ' + folder_name + ' doesn`t exist', button=['   OK   '], defaultButton='   OK   ')
			cmds.error()
	...
	def source_file(cls, source_folder, file_name):
		...
		if srs_file:
			...
			cmds.confirmDialog( title='Warning', message= 'File ' + source_folder + '/' + file_name + ' doesn`t exist', button=['   OK   '], defaultButton='   OK   ')
			cmds.error()
	...
	def silent_mode_selection(cls):
		...
		if selected:
			...
			cmds.confirmDialog( title='Warning', message= 'Select some mesh(es)', button=['   OK   '], defaultButton='   OK   ')
			cmds.error()
	...
	def silent_mode(cls, source_folder_name):
		...
		if not os.path.isfile(current_file_path):
			cmds.confirmDialog( title='Warning', message= 'Save file before', button=['   OK   '], defaultButton='   OK   ')
			cmds.error()

		if source_folder_name in current_file_path:
			cmds.confirmDialog( title='Warning', message= 'The open file is in the '+ source_folder_name +' directory ', button=['   OK   '], defaultButton='   OK   ')
			cmds.error()
...
class ImportReplace_Wnd(QDialog):
	...
	def filesList(self):
		...
		cmds.outlinerEditor( outliner_2, edit=True, mainListConnection='worldList', selectionConnection='modelList', showShapes=False, showReferenceNodes=False, showReferenceMembers=False, showAttributes=False, showConnected=False, showAnimCurvesOnly=False, autoExpand=False, showDagOnly=True, ignoreDagHierarchy=False, expandConnections=False, showNamespace=True, showCompounds=True, showNumericAttrsOnly=False, highlightActive=True, autoSelectNewObjects=False, doNotSelectNewObjects=False, transmitFilters=False, showSetMembers=False, setFilter='defaultSetFilter', ignoreHiddenAttribute=False, ignoreOutlinerColor=False )
	...
	def dirBrowser(self): #open browser to set export path
		...
		if bpath:
			...
			cmds.confirmDialog( title='Warning', message= 'Select dir with *.mb files', button=['   OK   '], defaultButton='   OK   ')
