...
class TrunkMenuAction(QWidgetAction):
    def __init__(self, parent, titleText, icon=None, options=True, widget=None):
        ...
        self.mainWidget = TrunkStandartWidget(self, title=titleText, icon=self.icon, options=self.options,
                                              widget=self.widget, actionName=self)
...
class TrunkStandartWidget(QWidget):
    ...
    def createUI(self):
        ...
        if self.options:
            self.option_icon = QPixmap(os.path.join(Utility.getCurrentDir(), 'source', 'options.svg')).scaled(11, 11,
