...
class BW_Export_Wnd(QDialog):
	...
	def export_command(self):
		...
		print 'EXPORT', self.export_path, self.selection

		sceneTextures = Utils.clearTextures()
		if cmds.optionVar(q = 'bw_export') == 1:
			arguments = 'noPrompt=1;exportMode='+ str(cmds.optionVar(q = 'bw_attr')) +';bumpMapped=1;keep_material=1;copyExternalTextures=1;copyTexturesTo'
			print 'ARG', arguments
			try:
				if self.selection:
