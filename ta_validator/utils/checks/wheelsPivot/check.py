		...
		for w in listWheels:
			if w.find("lod0") != -1:
				if bboxWheels(w) == True:
					...
					if ("%.4f" % w_coord[0]) != ("%.4f" % centroidCoords[0]) or ("%.4f" % w_coord[1]) != ("%.4f" % centroidCoords[1]) or ("%.4f" % w_coord[2]) != ("%.4f" % centroidCoords[2]):
						tmp = []
				...
				if lod0Wheels:
					for i in lod0Wheels:
						...
						if iShort == shortName:
							...
							if round(obj_pivot[0], 4) != round(w_pivot[0], 4) or round(obj_pivot[1], 4) != round(w_pivot[1], 4) or round(obj_pivot[2], 4) != round(w_pivot[2], 4):
								tmp = []
